    ...
    for i in range(128):                       # 128 patches in total
         for j in range(8):                    # each having 8 columns
                for k in range(8):             # and 8 rows
                    if (int(angle_patches[i][j][k]) in histogram):       # if an angle is one of our angles in our histogram, add its magnitude in the histogram respectively
                        histogram[int(angle_patches[i][j][k])] = histogram[int(angle_patches[i][j][k])] + mag_patches[i][j][k]
                    else:
                        distribution_angles = find_distribution_angles(int(angle_patches[i][j][k]), mag_patches[i][j][k], histogram)
